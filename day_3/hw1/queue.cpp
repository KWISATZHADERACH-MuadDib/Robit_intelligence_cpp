...
namespace yhw {
	...
	void queue<T>::clear() {			//queue 삭제
		if (this->data_ != nullptr) {	//queue가 존재할 경ㅇ우
			delete[] this->data_;		//data_ 동적 메로리 해제
	...
	queue<T> queue<T>::operator+(queue<T>& other) {
		...
		result.push(this->data_[front_ + i]);		//첫번째 queue 첫 원소부터 차레대로 result에 넣어준다.
		}
		for (size_t i = 0; i < other.size_; i++) {	//두번째 queue 첫 원소부터 차레대로 result에 넣어준다.
			result.push(other.data_[other.front_ + i]);
		}
	...
	queue<T>& queue<T>::operator=(const queue<T>& other) {
		if (this == &other) {			// 자기 자신에게 대입하는 경우 방지 (r = r; 같은 상황)
			return *this;
		}

		if (this->data_ != nullptr) {   // 기존에 갖고 있던 데이터 정리
			delete[] this->data_;
		}
		...
		if (other.data_ != nullptr) {
			this->data_ = new T[capacity_];               // 새로 배열 할당
			for (size_t i = 0; i < capacity_; i++) {
				this->data_[i] = other.data_[i];           // 값을 하나씩 실제로 복사
			}
		} else {
			this->data_ = nullptr;
		}
